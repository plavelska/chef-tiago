...
Deferred work from the Tiago Borges relaunch plan.

## Post-Launch Scope

- Refine the multi-page editorial prototype after the next visual/taste review.
- Revisit a contact form only if email-first contact does not filter inquiries well enough.
- Give music its own page, framed as food, room rhythm, gatherings, and atmosphere rather than a generic DJ portfolio.
- Consider a static-site generator only if editing the flat files becomes painful after launch.
- Consider an external image CDN only if optimized committed derivatives are not enough for performance.

## Pre-Launch Follow-Ups

- Completed: selected, optimized, and converted public image derivatives from `new assets/`; keep the raw source folder uncommitted.
